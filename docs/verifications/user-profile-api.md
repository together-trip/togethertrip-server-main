...
검증 범위는 내 정보 조회/수정/탈퇴, 내 여행 참여자 조회, 비활성 사용자 차단, soft delete 처리, `@SQLRestriction` 컴파일 호환성이다.
...
Kotlin 컴파일, Spring test context, `UserServiceTest`, 기존 `MainApplicationTests`가 통과했다.
