...
class RedisOAuthSignupLockIntegrationTest @Autowired constructor(
    ...
    private val providerUserIds = mutableListOf<String>()

    @AfterEach
    fun cleanRedisKeys() {
        redisTemplate.delete(providerUserIds.map(::key))
    }

    @Test
    fun `가입 lock 16개가 동시에 경쟁하면 하나만 block에 진입한다`() {
        val providerUserId = providerUserId("concurrent")
        ...
        try {
            val futures = (0 until contenderCount).map {
                executor.submit<String> {
                    ...
                    try {
                        lock.withLock(OAuthProvider.KAKAO, providerUserId) {
            ...
            assertTrue(redisTemplate.hasKey(key(providerUserId)))
            ...
            assertFalse(redisTemplate.hasKey(key(providerUserId)))
        } finally {
            releaseWinner.countDown()
    ...
    fun `lock 소유 token이 바뀌면 finally의 Lua release가 다른 소유자 key를 삭제하지 않는다`() {
        val providerUserId = providerUserId("owner-safety")

        lock.withLock(OAuthProvider.KAKAO, providerUserId) {
            redisTemplate.opsForValue().set(key(providerUserId), "replacement-owner")
        }

        assertEquals("replacement-owner", redisTemplate.opsForValue().get(key(providerUserId)))
    }

    @Test
    fun `활성 transaction에서는 commit 이후에 lock을 해제한다`() {
        val providerUserId = providerUserId("transaction")
    ...
    private fun providerUserId(suffix: String): String {
        return "integration-$suffix".also(providerUserIds::add)
    }

    private fun key(providerUserId: String): String {
        return "auth:oauth-signup-lock:${OAuthProvider.KAKAO}:$providerUserId"
    }
}
